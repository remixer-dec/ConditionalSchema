...
class ConditionalModel(BaseModel, metaclass=ConditionalModelMeta):
  ...
  def _add_inactive_fields_to_schema(
    ...
    for field_name, cond_info in conditional_fields.items():
      ...
      # IMPORTANT: Skip fields excluded by bind-time conditions
      # fill_inactive should only fill fields excluded by runtime conditions,
      # not fields excluded by .bind()
      if not cond_info.bound_active_result:
        continue
      ...
      # Priority: dict > explicit global > field default > None
      if isinstance(inactive_default, dict):
        # Per-field default values
        default_value = inactive_default.get(field_name, None)
      elif inactive_default is not None:
        # Explicitly provided global default (overrides field default)
        default_value = inactive_default
      elif cond_info.default is not ...:
